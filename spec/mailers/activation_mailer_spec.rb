...
RSpec.describe ActivationMailer, type: :mailer do
  describe 'activation_email' do
    before(:each) do
      @user = create(:user)
      @email = described_class.activation_email(@user).deliver_now
    end
    ...
    it 'has a body which invites user to click link to activate account' do
      expect(@email.body.encoded).to have_content('Visit here to activate your account')
    end
    ...
    it 'correctly interpolates the URL of the activation page' do
      expect(@email.body.encoded).to have_link('here', href: "http://localhost:3000/users/#{@user.id}/activate")
    end
  end
end
